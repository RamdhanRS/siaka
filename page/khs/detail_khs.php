...
if ($nim && $tahun_akademik && $semester) {
  ...
  $query = "SELECT
            ...
            WHERE krs.status = 'disetujui'
              AND m.nim = '$nim_safe'
              AND krs.tahun_akademik = '$tahun_akademik'
              AND krs.semester = '$semester_safe'
            ORDER BY mk.nama_mk";
  ...
  $query_count = "SELECT 
                  ...
                  -- IPS semester yang diminta
                  (
                    SELECT 
                      ROUND(SUM(
                        CASE 
                          WHEN khs2.nilai_angka >= 85 THEN 4.0 * mk2.sks
                          WHEN khs2.nilai_angka >= 75 THEN 3.0 * mk2.sks
                          WHEN khs2.nilai_angka >= 65 THEN 2.0 * mk2.sks
                          WHEN khs2.nilai_angka >= 50 THEN 1.0 * mk2.sks
                          ELSE 0
                        END
                      ) / NULLIF(SUM(mk2.sks), 0), 2)
                    FROM khs khs2
                    JOIN krs krs2 ON khs2.id_krs = krs2.id
                    JOIN jadwal j2 ON krs2.id_jadwal = j2.id
                    JOIN mata_kuliah mk2 ON j2.id_mk = mk2.id
                    WHERE krs2.status = 'disetujui'
                      AND krs2.id_mahasiswa = m.id
                      AND krs2.tahun_akademik = '$tahun_safe'
                      AND krs2.semester = '$semester'
                  ) AS ips_semester,
                  ...
                  ROUND(SUM(
                    CASE 
                      ...
                      ELSE 0
                    END
                  ) / NULLIF(SUM(mk.sks), 0), 2) AS ipk_total
                ...
                GROUP BY m.nim, m.nama_lengkap, p.nama_prodi";
  ...
  $query_detail = "SELECT 
                  ...
                  JOIN jadwal j ON krs.id_jadwal = j.id
                  JOIN mata_kuliah mk ON j.id_mk = mk.id
                  JOIN prodi p ON p.id = mk.id_prodi
                  JOIN fakultas f ON f.id = p.id_fakultas
                  JOIN dosen d ON j.id_dosen = d.id
                  WHERE krs.status = 'disetujui'
                    AND m.nim = '$nim'
                    AND krs.tahun_akademik = '$tahun_safe'
